import React, { useState } from 'react';
import { 
  ArrowRight, 
  HelpCircle, 
  Phone, 
  MapPin, 
  Clock, 
  CheckCircle,
  ChevronDown,
  Info
} from 'lucide-react';

export default function PassportApplicationCn() {
  const [currentStep, setCurrentStep] = useState(1);
  const [fontSize, setFontSize] = useState(18);
  const [showHelp, setShowHelp] = useState(false);

  const adjustFontSize = (change) => {
    const newSize = Math.min(24, Math.max(14, fontSize + change));
    setFontSize(newSize);
  };

  const locations = [
    {
      name: "UTC 吉隆坡",
      address: "Jalan Raja Laut, 50350 Kuala Lumpur",
      features: ["轮椅通道", "老年人优先通道", "指导协助"],
      distance: "2.5公里"
    },
    {
      name: "PPA 布城",
      address: "Presint 2, 62550 Putrajaya",
      features: ["盲文指南", "听力辅助", "舒适座椅"],
      distance: "15公里"
    },
    {
      name: "城市转型中心 (UTC) 新山",
      address: "Jalan Abdullah Ibrahim, 80000 Johor Bahru",
      features: ["轮椅通道", "老年人优先通道"],
      distance: "8公里"
    }
  ];

  const requirements = [
    "原始身份证 (及复印件)",
    "2张护照照片 (白色背景)",
    "旧护照 (用于续签)",
    "付款确认单"
  ];

  const steps = [
    "填写在线表格",
    "上传文件",
    "完成付款",
    "预约时间",
    "访问移民局"
  ];

  return (
    <div className="min-h-screen bg-gray-50" style={{ fontSize: `${fontSize}px` }}>
      {/* 辅助工具栏 */}
      <div className="fixed bottom-6 right-6 z-50 flex gap-3">
        <button 
          onClick={() => setShowHelp(!showHelp)}
          className="w-14 h-14 flex items-center justify-center bg-blue-600 rounded-full shadow-lg hover:bg-blue-700 text-white"
          aria-label="获取帮助"
        >
          <HelpCircle className="h-6 w-6" />
        </button>
      </div>

      {/* 帮助面板 */}
      {showHelp && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-40 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-6 max-w-md w-full">
            <h3 className="text-xl font-bold text-gray-900 mb-4">需要帮助？</h3>
            <div className="space-y-4">
              <div className="flex items-start">
                <Phone className="h-5 w-5 text-blue-500 mt-1 mr-3 flex-shrink-0" />
                <div>
                  <h4 className="font-medium text-gray-900">电话支持</h4>
                  <p className="text-gray-600">03-8000 8000 (每天上午8点至晚上10点)</p>
                </div>
              </div>
              <div className="flex items-start">
                <MapPin className="h-5 w-5 text-blue-500 mt-1 mr-3 flex-shrink-0" />
                <div>
                  <h4 className="font-medium text-gray-900">现场帮助</h4>
                  <p className="text-gray-600">访问任何 UTC 获取指导申请</p>
                </div>
              </div>
              <div className="flex items-start">
                <Info className="h-5 w-5 text-blue-500 mt-1 mr-3 flex-shrink-0" />
                <div>
                  <h4 className="font-medium text-gray-900">低收入援助</h4>
                  <p className="text-gray-600">B40 申请者可享受折扣</p>
                </div>
              </div>
            </div>
            <button 
              onClick={() => setShowHelp(false)}
              className="mt-6 w-full py-2 bg-gray-100 hover:bg-gray-200 rounded-lg font-medium"
            >
              关闭
            </button>
          </div>
        </div>
      )}

      <div className="max-w-6xl mx-auto px-4 py-8">
        {/* 标题 */}
        <header className="text-center mb-10">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3">
            马来西亚护照申请
          </h1>
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">
            简单步骤获取护照，提供老年人友好协助
          </p>
        </header>

        {/* 进度步骤 */}
        <div className="mb-10">
          <div className="flex justify-between relative">
            {steps.map((step, index) => (
              <div key={index} className="flex flex-col items-center z-10">
                <div 
                  className={`w-12 h-12 rounded-full flex items-center justify-center ${
                    currentStep > index ? 'bg-green-500 text-white' : 
                    currentStep === index ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-600'
                  }`}
                >
                  {currentStep > index ? <CheckCircle className="h-6 w-6" /> : index + 1}
                </div>
                <span className={`mt-2 text-sm text-center ${
                  currentStep >= index ? 'font-medium text-gray-900' : 'text-gray-500'
                }`}>
                  {step}
                </span>
              </div>
            ))}
            <div className="absolute top-6 left-0 right-0 h-1 bg-gray-200 -z-10">
              <div 
                className="h-full bg-blue-500 transition-all duration-300" 
                style={{ width: `${(currentStep - 1) * 25}%` }}
              ></div>
            </div>
          </div>
        </div>

        {/* 当前步骤内容 */}
        <div className="bg-white rounded-xl shadow-sm p-6 md:p-8 mb-8">
          {currentStep === 1 && (
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-6">个人信息</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-gray-700 mb-2">全名 (与身份证一致)</label>
                  <input 
                    type="text" 
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400"
                    placeholder="例如：Ahmad bin Abdullah"
                  />
                </div>
                <div>
                  <label className="block text-gray-700 mb-2">身份证号码</label>
                  <input 
                    type="text" 
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400"
                    placeholder="例如：700101-01-1234"
                  />
                </div>
                <div>
                  <label className="block text-gray-700 mb-2">电话号码</label>
                  <input 
                    type="tel" 
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400"
                    placeholder="例如：012-345 6789"
                  />
                </div>
                <div>
                  <label className="block text-gray-700 mb-2">电子邮件地址</label>
                  <input 
                    type="email" 
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400"
                    placeholder="your.email@example.com"
                  />
                </div>
              </div>
              <div className="mt-6">
                <label className="flex items-start">
                  <input type="checkbox" className="mt-1 mr-2" />
                  <span className="text-gray-700">我符合低收入 (B40) 折扣资格</span>
                </label>
              </div>
            </div>
          )}

          {currentStep === 2 && (
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-6">文件上传</h2>
              <div className="space-y-6">
                {requirements.map((item, index) => (
                  <div key={index} className="border border-gray-200 rounded-lg p-4">
                    <h3 className="font-medium text-gray-900 mb-2">{item}</h3>
                    <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
                      <p className="text-gray-500 mb-2">点击上传 {item.toLowerCase()}</p>
                      <button className="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg font-medium">
                        选择文件
                      </button>
                    </div>
                  </div>
                ))}
              </div>
            </div>
          )}

          {currentStep === 3 && (
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-6">付款选项</h2>
              <div className="space-y-4">
                <div className="border border-gray-200 rounded-lg p-4 hover:border-blue-400 cursor-pointer">
                  <h3 className="font-medium text-gray-900 mb-1">标准 (5年有效期)</h3>
                  <p className="text-gray-600">RM200 (B40 申请者 RM150)</p>
                </div>
                <div className="border border-gray-200 rounded-lg p-4 hover:border-blue-400 cursor-pointer">
                  <h3 className="font-medium text-gray-900 mb-1">老年人折扣</h3>
                  <p className="text-gray-600">RM100 (适用于60岁以上申请者)</p>
                </div>
                <div className="border border-gray-200 rounded-lg p-4 hover:border-blue-400 cursor-pointer">
                  <h3 className="font-medium text-gray-900 mb-1">快速处理</h3>
                  <p className="text-gray-600">+RM100 (2个工作日内完成)</p>
                </div>
              </div>
              <div className="mt-6 p-4 bg-blue-50 rounded-lg">
                <h3 className="font-medium text-gray-900 mb-2">付款方式</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <button className="p-3 bg-white rounded-lg border border-gray-200 hover:border-blue-400">
                    信用卡/借记卡
                  </button>
                  <button className="p-3 bg-white rounded-lg border border-gray-200 hover:border-blue-400">
                    网上银行
                  </button>
                  <button className="p-3 bg-white rounded-lg border border-gray-200 hover:border-blue-400">
                    Touch 'n Go
                  </button>
                  <button className="p-3 bg-white rounded-lg border border-gray-200 hover:border-blue-400">
                    柜台付款
                  </button>
                </div>
              </div>
            </div>
          )}

          {currentStep === 4 && (
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-6">预约时间</h2>
              <div className="mb-6">
                <label className="block text-gray-700 mb-2">选择地点</label>
                <div className="relative">
                  <select className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 appearance-none">
                    {locations.map((loc, index) => (
                      <option key={index} value={index}>
                        {loc.name} ({loc.distance} away)
                      </option>
                    ))}
                  </select>
                  <ChevronDown className="h-5 w-5 text-gray-400 absolute right-3 top-3.5 pointer-events-none" />
                </div>
              </div>
              
              <div className="space-y-4">
                {locations[0].features.map((feature, index) => (
                  <div key={index} className="flex items-center">
                    <CheckCircle className="h-5 w-5 text-green-500 mr-2" />
                    <span className="text-gray-700">{feature}</span>
                  </div>
                ))}
              </div>

              <div className="mt-6">
                <label className="block text-gray-700 mb-2">选择日期和时间</label>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {['Mon 10/7', 'Tue 11/7', 'Wed 12/7', 'Thu 13/7'].map((date, index) => (
                    <button 
                      key={index}
                      className="p-3 bg-white rounded-lg border border-gray-200 hover:border-blue-400 text-center"
                    >
                      {date}
                      <div className="text-sm text-gray-500 mt-1">9AM-12PM</div>
                    </button>
                  ))}
                </div>
              </div>
            </div>
          )}

          {currentStep === 5 && (
            <div className="text-center">
              <CheckCircle className="h-16 w-16 text-green-500 mx-auto mb-6" />
              <h2 className="text-2xl font-bold text-gray-900 mb-3">申请已提交！</h2>
              <p className="text-gray-600 mb-6 max-w-md mx-auto">
                您的护照申请已收到。请携带所有原始文件到您的预约地点。
              </p>
              <div className="bg-blue-50 rounded-lg p-4 mb-6">
                <h3 className="font-medium text-gray-900 mb-2">您的预约</h3>
                <p className="text-gray-700">UTC 吉隆坡</p>
                <p className="text-gray-700">2023年7月10日星期一 上午10:00</p>
                <p className="text-gray-700">老年人优先通道</p>
              </div>
              <button className="px-6 py-3 bg-blue-500 hover:bg-blue-600 text-white rounded-lg font-medium">
                打印确认
              </button>
            </div>
          )}

          <div className="mt-10 flex justify-between">
            {currentStep > 1 && currentStep < 5 && (
              <button 
                onClick={() => setCurrentStep(currentStep - 1)}
                className="px-6 py-3 bg-gray-200 hover:bg-gray-300 text-gray-800 rounded-lg font-medium"
              >
                返回
              </button>
            )}
            {currentStep < 5 && (
              <button 
                onClick={() => setCurrentStep(currentStep + 1)}
                className="ml-auto px-6 py-3 bg-blue-500 hover:bg-blue-600 text-white rounded-lg font-medium"
              >
                继续
              </button>
            )}
            {currentStep === 5 && (
              <button 
                onClick={() => window.location.href = '/'}
                className="ml-auto px-6 py-3 bg-blue-500 hover:bg-blue-600 text-white rounded-lg font-medium"
              >
                返回首页
              </button>
            )}
          </div>
        </div>

        {/* 帮助部分 */}
        <div className="bg-white rounded-xl shadow-sm p-6">
          <h2 className="text-xl font-bold text-gray-900 mb-4">需要帮助？</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-4 bg-blue-50 rounded-lg">
              <div className="flex items-center mb-2">
                <Phone className="h-5 w-5 text-blue-500 mr-2" />
                <h3 className="font-medium text-gray-900">电话支持</h3>
              </div>
              <p className="text-gray-600">03-8000 8000 (每天上午8点至晚上10点)</p>
            </div>
            <div className="p-4 bg-blue-50 rounded-lg">
              <div className="flex items-center mb-2">
                <MapPin className="h-5 w-5 text-blue-500 mr-2" />
                <h3 className="font-medium text-gray-900">现场帮助</h3>
              </div>
              <p className="text-gray-600">访问任何 UTC 获取指导申请</p>
            </div>
            <div className="p-4 bg-blue-50 rounded-lg">
              <div className="flex items-center mb-2">
                <Clock className="h-5 w-5 text-blue-500 mr-2" />
                <h3 className="font-medium text-gray-900">老年人优先时间</h3>
              </div>
              <p className="text-gray-600">工作日早上9点至11点优先服务老年人</p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}